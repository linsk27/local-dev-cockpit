...
Dev Cockpit 的 npm CLI 入口。它会在本机启动一个 Vue 3 面板，用来恢复本地开发现场：项目列表、启动命令、Git 状态、端口、日志、诊断、AI 上下文和 Resource Radar 资源库。
...
Resource Radar 支持粘贴 GitHub、Demo、工具、教程、Prompt、MCP 或视频号文本线索，先生成结构化预览卡，确认后再写入本地资源库。默认使用本地规则；配置 OpenAI-compatible API Key 后，导入时会自动走 AI 结构化解析。
