...
资源库支持导出 / 导入 JSON，适合在多台电脑或不同应用之间同步同一批资源。导出格式固定为：
...
AI 配置页用于选择供应商、填写 Base URL、模型和 API Key，并进行连接测试。配置只保存在本机，不会写入仓库。
...
- **项目控制台**：恢复本地项目现场，覆盖扫描、命令、端口、日志、Git、Python/Conda 环境、AI 上下文和桌面版。
- **资源雷达**：沉淀 AI skills、工具、Demo、教程、Prompt、MCP 和视频号笔记线索；导入时先抓取元数据和图片，再通过本地规则或可选 AI 生成预览卡，确认后才入库。

现在适合做小范围试用和反馈收集。继续优化会优先围绕三件事：真实项目启动成功率、资源导入质量、桌面安装/更新体验；不再为了“看起来功能多”继续堆入口。
