...
- 首页只选择一个具体根目录作为当前工作区；项目扫描、状态刷新和性能指标都限定在该根目录内。
...
- 外部运行：server 读取 Windows 当前监听端口和进程命令行；如果命令行包含项目路径，就把该端口归属到项目。
...
- 性能观测：`/api/performance?rootId=...` 返回进程内存、CPU 采样和最近扫描状态；该接口不触发扫描，前端用“占用低/中/高”给用户做语义化展示。
