...
- `packages/core` 只负责识别项目类型和推荐命令，例如 Python 的 `python -m flask --app ... run` / `python -m uvicorn ...`、Maven 的 `spring-boot:run`、Gradle 的 `bootRun`、Laravel 的 `artisan serve`、Rails 的 `rails server`、.NET 的 `dotnet run`。
